...
class SearchableBuilderTest extends TestCase
{
    ...
    public function it_moves_wheres_to_subquery_for_performance_if_possible()
    {
        ...
        $model->getConnection()
            ...
            ->once()
            ->with($query, $bindings, Mockery::any())
            ->andReturn([]);

        $model
            ->whereHas('profile', function ($q) {
                $q->where('id', 5);
            }) // where with subquery - not moved
    ...
    public function length_aware_pagination()
    {
        ...
        $model = $this->getModel();
        $model->getConnection()->shouldReceive('select')->once()->with($query, $bindings, Mockery::any())->andReturn([]);
