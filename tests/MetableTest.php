...
class MetableTest extends TestCase
{
    ...
    public function meta_whereExists()
    {
        ...
        $model = $this->getModel();
        $model->getConnection()->shouldReceive('raw')->once()->andReturnUsing(function ($value) {
            return new Expression($value);
        });

        $query = $model->whereExists(function ($q) {
            $q->selectRaw(1)->where('size', '>', 10);
        });
    ...
    public function meta_whereBetween_numeric()
    {
        ...
        $model = $this->getModel();
        $model->getConnection()->shouldReceive('raw')->twice()->andReturnUsing(function ($value) {
            return new Expression($value);
        });
